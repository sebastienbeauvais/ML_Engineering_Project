...
SELECT
    ...
    , AVG(hts.home_hit2plate_ratio) OVER (PARTITION BY hts.home_team ORDER BY htw.game_id RANGE BETWEEN
	20 PRECEDING AND 1 PRECEDING) AS home_20_game_hit2plate_avg
    , AVG(hts.home_safe2plate_ratio) OVER (PARTITION BY hts.home_team ORDER BY htw.game_id RANGE BETWEEN
	20 PRECEDING AND 1 PRECEDING) AS home_20_game_safe2plate_avg
    ...
    , AVG(ats.away_hit2plate_ratio) OVER (PARTITION BY ats.away_team ORDER BY htw.game_id RANGE BETWEEN
	20 PRECEDING AND 1 PRECEDING) AS away_20_game_hit2plate_avg
    , AVG(ats.away_safe2plate_ratio) OVER (PARTITION BY ats.away_team ORDER BY htw.game_id RANGE BETWEEN
	20 PRECEDING AND 1 PRECEDING) AS away_20_game_safe2plate_avg
    , htw.HomeTeamWins
FROM
    home_team_stats hts
...
SELECT
    ...
    , (home_20_game_hit2plate_avg - away_20_game_hit2plate_avg) AS diff_hit2plate_avg
    , (home_20_game_safe2plate_avg - away_20_game_safe2plate_avg) AS diff_safe2plate_avg
    , HomeTeamWins
FROM
    baseball_feats;
